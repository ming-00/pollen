class CommentforumsController < ApplicationController
    ...
    def create
        ...
        if @commentforum.save
            ...
            (User.joins(:commentforums).where(commentforums: {forumpost_id: @forumpost.id}).uniq - [current_user]).uniq.each do |user|
                Notification.create(title: @forumpost.title, recipient: user, actor: current_user, action: "added", notifiable: @commentforum)
            end
            Notification.create(title: @forumpost.title, recipient: @forumpost.user, actor: current_user, action: "added", notifiable: @commentforum)
    ...
    def markaccepted
        ...
        if @commentforum.accepted == false
            Notification.create(title: @commentforum.forumpost.title, recipient: @commentforum.user, actor: current_user, action: "accepted", notifiable: @commentforum)
